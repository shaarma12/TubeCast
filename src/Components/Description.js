...
const Description = ({ data = {}, subscribe }) => {
  ...
  return (
    <div
      className="w-[57.5rem] bg-[#FFFFFF1A] rounded-xl p-3 mt-4 cursor-pointer"
      onClick={() => {
        setToggle(!toggle);
      }}
    >
        ...
        <div>
          ...
          <div className="flex mt-5">
            ...
            <div className="text-white">
              <p className="text-xl font-bold text-white -mt-[0.1rem]">
                {data?.channelTitle.length > 20
                  ? data?.channelTitle.slice(0, 19) + "..."
                  : data?.channelTitle}
              </p>
              <p className="text-[#AAAAAA] text-sm -mt-[0.15rem]">
                {viewCount(subscribe)} subscribers
              </p>
            </div>
          </div>
          <p className="font-semibold text-white mt-8">Show less</p>
        </div>
      ) : (
        <div
          className="text-white font-semibold"
          dangerouslySetInnerHTML={{
            __html:
              sanitizedData.length > 245
                ? sanitizedData.slice(0, 244) + "  ...more"
