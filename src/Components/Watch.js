...
const Watch = () => {
  ...
  return (
    <>
      {flag && (
        <div
          className="fixed inset-0 bg-black bg-opacity-50 ml-[15.2rem]"
          onClick={() => {
      ...
      <div className="flex justify-between gap-6 mt-4">
        <div className="bg-[#212121] ml-[6.5rem] mb-40">
          ...
          ></iframe>
          <p className="text-white text-xl font-semibold w-[50rem] -mt-4">
            ...
            <div className="flex">
              ...
              <div className="text-white">
                ...
                <p className="text-[#AAAAAA] text-sm -mt-[0.15rem]">
                  {formatCommentCount(
                    subscribe?.items[0]?.statistics?.subscriberCount
                  )}{" "}
                  subscribers
                </p>
              </div>
              <button className="bg-[#F1F1F1] rounded-full w-[6.5rem] h-9 ml-6 font-medium mt-1 hover:opacity-85">
                Subscribe
              </button>
            </div>
                ...
                {data?.videoData?.statistics?.likeCount && (
                  <button className="flex justify-center bg-[#FFFFFF1A] items-center w-[5.5rem] h-9 rounded-tl-3xl rounded-bl-3xl hover:bg-[#ffffff35] border-r-2 border-[#ffffff35]">
                    ...
                    <p className="text-white font-semibold">
                      {formatLikeCount(data?.videoData?.statistics?.likeCount)}
                    </p>
                  </button>
                )}
          ...
          <Description
            data={data?.videoData?.snippet}
            subscribe={subscribe?.items[0]?.statistics?.subscriberCount}
          />
          <div className="flex">
            <p className="text-white font-bold text-xl mb-10 mt-6">
              {formatCommentCount(data?.videoData?.statistics?.commentCount)}{" "}
              Comments
            </p>
            <div className="relative z-10">
              ...
              >
                <img
                  src={sorting}
                  alt="sortingIcon"
                  className="h-8 w-7 mt-5 mr-3"
                />
                <p className="text-white mb-5 mt-6 font-semibold">Sort by</p>
              </div>
              {sortBy && (
                <div
                  className="bg-[#3e3e3e] w-[8.5rem] h-28 flex flex-col items-center absolute left-10 justify-center rounded-lg z-20"
                  ref={dropdownRef}
                >
                  <Link
                    className={`${
                      !sortData
                        ? "text-white font-normal px-[0.85rem] py-[0.6rem] bg-[#ffffff3d] mb-2"
                        : "text-white font-normal pb-5 hover:px-[0.85rem] hover:py-[0.6rem] hover:bg-[#ffffff18] mt-2"
                    }`}
                  >
                    <p
                      onClick={() => {
                  ...
                  <Link
                    className={`${
                      sortData
                        ? "text-white font-normal pb-2  px-[1.5rem] py-[0.6rem] bg-[#ffffff3d] mb-3"
                        : "text-white font-normal pb-2 mr-3 hover:px-[1.5rem] hover:py-[0.6rem] hover:bg-[#ffffff18] hover:mb-3 hover:mr-0"
                    }`}
                  >
                    <p
                      onClick={() => {
            ...
            {sortData
              ? comm?.items
                  ...
                  .map((data) => (
                    <Comments
                      key={data?.id}
                      commentData={data?.snippet?.topLevelComment?.snippet}
                    />
                  ))
              : comm?.items
                  ?.slice()
        ...
        <div>
          <div className="w-[25rem]">
            {Isshorts > 3 && (
              <div className="flex">
                <img
                  src="https://upload.wikimedia.org/wikipedia/commons/thumb/f/fc/Youtube_shorts_icon.svg/724px-Youtube_shorts_icon.svg.png"
                  alt="YTShorts"
                  className="mt-1 w-5 h-6 mr-2"
                />
