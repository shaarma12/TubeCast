...
const Shorts = ({ channelId, setIsShorts }) => {
  ...
  const getShortsVideo = async () => {
    const rep = await fetch(
      YOUTUBE_SHORTS_API + channelId?.videoData?.snippet?.channelId
    );
        ...
        <div className="flex items-center justify-center mt-2">
          <img
            ...
            className={`${
              currentIndex === 0
                ? "hidden"
                : "w-10 h-10 absolute left-[64.5rem] cursor-pointer hover:-scale-y-105"
            }`}
          ...
          <img
            ...
            alt="rightArrow"
            className={`${
              currentIndex + 3 >= shorts.length - 3
                ? "hidden"
                : "w-10 h-10 absolute left-[89rem] cursor-pointer hover:-scale-y-105"
            }`}
            onClick={handleNext}
          />
        </div>
